...
public interface CustomerEPDoc {

    static final String api_group_customers = "Customer endpoint";
    ...
    @Operation(
        tags = {api_group_customers},
        summary = "Return Customer by Name, Name must not be empty.",
        description = "Return Customer if Name exists, else return error 404 (not found)."
        // parameters = {@Parameter(name="id", in=ParameterIn.QUERY, schema=@Schema(implementation=Long.class)) }
    )
    @ApiResponses(value={
        @ApiResponse(responseCode="200", description="OK", content=@Content(mediaType="application/json")),
