...
export default function PageRegistrationWorker() {
  ...
  const handleRegistration = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await registrationWorker(
        name,
        address,
        email,
        password,
  ...
  return (
    <div className="animated-background">
      ...
      >
        <MDBCard className="worker-registration-container ">
          <MDBCardBody className="">
            <h2 className="text-uppercase text-center mb-5">
              Registrieren als Worker
            </h2>
            <form onSubmit={handleRegistration}>
              ...
              <MDBInput
                ...
                onBlur={() =>
                  handleAddressValidation(address).then((valid) =>
              ...
              />
              {!addressValid && (
                <div style={{ color: "#e4a11b" }}>Ungültige Adresse.</div>
              )}
              <MDBInput
                wrapperClass="mb-4"
                label="Deine E-Mail"
              ...
              <div style={{ color: "white", marginBottom: "10px" }}>
                Das Passwort muss mindestens einen Großbuchstaben, eine Zahl und
              ...
              <select
                className="form-select mb-4 option-black"
                value={jobType}
                ...
                {jobList.map((job, index) => (
                  <li key={index} style={{ color: "white" }}>
                    ...
                    <MDBIcon
                      ...
                      onClick={() => handleJobTypeRemove(job)}
                    />
                  </li>
                ))}
              </ul>
              <label className="form-label" style={{ color: "white" }}>
                Gehaltswunsch (Ab welchem Gehalt sollen wir dir Aufträge
                zusenden?)
              </label>
              ...
              <MDBCheckbox
                ...
                label={
                  <span>
                    Ich stimme den{" "}
                    <Link to="/agb" className="text-white">
                      Nutzungsbedingungen
                    </Link>{" "}
                    zu
                  </span>
